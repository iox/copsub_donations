class FrontController < ApplicationController
  ...
  def report
    ...
    @almost_donated = Donor.where("last_donated_at IS NULL OR DATE(last_donated_at) < filled_donation_form_date").where("filled_donation_form_date > ?", 3.months.ago).where("selected_amount > 0").order("filled_donation_form_date desc")
  end
