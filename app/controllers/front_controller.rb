class FrontController < ApplicationController
  ...
  def report
    ...
    @almost_donated = Donor.where("updated_at >= ? AND updated_at <= ?", @start_date, @end_date).where("last_donated_at IS NULL OR last_donated_at < ?", 4.months.ago).where("selected_amount > 0").order("updated_at desc")
  end
