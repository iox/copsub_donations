class FrontController < ApplicationController
  ...
  def report
    ...
    @almost_donated = Donor.where("filled_donation_form_date >= ? AND filled_donation_form_date <= ?", @start_date, @end_date).where("last_donated_at IS NULL OR last_donated_at < ?", 4.months.ago).where("selected_amount > 0").order("filled_donation_form_date desc")
  end
