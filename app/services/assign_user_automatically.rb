class AssignUserAutomatically
  ...
  def try_to_assign_user
    ...
    if @donation.agilecrm_id.blank?
      donation = search_for_similar_assigned_donation
      @donation.agilecrm_id = donation.agilecrm_id if donation
    end
    ...
    if @donation.agilecrm_id.blank? && !@donation.bank_reference.blank?
      ...
      if users.size == 1
        @donation.agilecrm_id = users.first.id
      elsif users.size > 1
  ...
  def search_for_similar_assigned_donation
    scope = Donation.where("agilecrm_id IS NOT NULL")
